...
export const MagicTextReveal: React.FC<MagicTextRevealProps> = ({
  ...
  const createParticles = useCallback((
    ...
    // Calculate spread area for initial scattering
    const textWidth = maxX - minX;
    const textHeight = maxY - minY;
    const spreadRadius = Math.max(textWidth, textHeight) * 0.15;
    
    // Second pass: create particles with scattered initial positions
    for (let y = 0; y < canvas.height; y += sampleRate) {
      for (let x = 0; x < canvas.width; x += sampleRate) {
        const index = (y * canvas.width + x) * 4;
        const alpha = data[index + 3];
        if (alpha > 0) {
          ...
          // Start particles scattered (text invisible initially)
  ...
  const updateParticles = useCallback((
    ...
    setHasBeenShown: (shown: boolean) => void,
    spread: number,
    speed: number
  ) => {
    ...
    particles.forEach(particle => {
      if (hasBeenShown) {
        // After text has been shown, fade out particles completely
        particle.opacity = Math.max(0, particle.opacity - FADE_SPEED * deltaTime);
      } else if (isHovered) {
        // When hovered for the first time, particles return to original position
        ...
        // Keep particles visible while forming text
        particle.opacity = Math.min(particle.originalAlpha, particle.opacity + FADE_SPEED * deltaTime);
      } else {
    ...
    // Show text on first hover and keep it visible forever
    if (isHovered && !hasBeenShown) {
      setShowText(true);
      setHasBeenShown(true);
    }
    // Once shown, text stays visible
    if (hasBeenShown) {
      setShowText(true);
    }
  }, []);
  ...
  }, [isHovered, hasBeenShown, spread, speed, globalDpr, updateParticles, renderParticles]);
  ...
  return (
    <div
      ref={wrapperRef}
      className={cn("relative flex items-center justify-center overflow-hidden rounded-lg transition-all duration-300", className)}
      style={{
    ...
    >
      {/* Animated text that appears when hovered */}
      <div
